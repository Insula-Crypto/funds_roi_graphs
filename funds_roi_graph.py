...
def main():
    ...
    get_fund_GAV_ROI('0x0d9596Afc608B3322A17118A573750045F52C0B8', 'Malta.csv')
...
def get_fund_GAV_ROI(address, file_name):    
    ...
    investmentTx.loc[:, 1] = pd.to_datetime(investmentTx[1]).dt.strftime('%Y-%m-%d')
    investmentTx.rename(columns={1:'Date', 4:'side', 6:'amount'}, inplace=True)

    rng = pd.date_range(investmentTx.iloc[-1]['Date'], datetime.now())

    investmentTx.loc[investmentTx['side'] == 'to', 'amount'] = investmentTx['amount'] * -1
    investmentTx['Date'] = pd.to_datetime(investmentTx['Date'])

    a = investmentTx['amount'].groupby(investmentTx['Date'].dt.to_period('D')).sum().to_frame()
    a = a.set_index(a.index.to_timestamp())
    a = a.reindex(rng, fill_value=0)

    a.amount = (a['amount'].cumsum())

    sent = requests.get('https://api.bloxy.info/widget/address_value_daily?address=' + address + '&currency=ETH&key=ACCunOMWYpmCp&format=table&price_currency=ETH').json()
    df2 = pd.DataFrame(sent)
    df2.rename(columns={0:'Date', 4: 'GAV'}, inplace=True)
    df2 = df2.set_index('Date')

    data = pd.concat([a, df2], axis=1, sort=False)
    data['ROI-ETH'] = (data['GAV'] / data['amount'] - 1) * 100
    ...
    df = pd.concat([df1, data], axis=1, sort=False)[['ROI-USD', 'ROI-ETH']]
    # df = df.set_index('Date')
    # df.index = pd.to_datetime(df.index)
